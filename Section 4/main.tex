...
Consider an  American lookback call option (fixed strike), i.e.  $\Phi(\SSS_t) = (\max_{u\in[0,t]} S_u - K)^+$,  in the Black-Scholes world.  %$\Phi(\SSS_t) = (\gamma \max_{u\in[0,t]} S_u - S_t)^+$,
...
To approximate the American price, we use a regular partition  with $N=200$ exercise dates prior to maturity. % giving  $\calT_{\!N} = \{\frac{n}{2N} \ | \ n=0,\ldots,N\}$. 
...
%}
%$\frac{D_{0,t'_{m}}\varphi(X_{t_m'})-D_{0,t_{n}}\varphi(X_{t_n})}{D_{0,t'_{m}}\varphi(X_{t_m'})} = 1-e^{-r(t_n - t'_{m})}  \approx \frac{1}{N}$.   We use importance sampling with $\lambda = \frac{r-\delta}{\sigma}$
